...
async function copyFilteredKeys() { const button = $("#copy-filtered-keys"); button.disabled = true; try { const params = new URLSearchParams({ status: state.keys.status }); const data = await api(`/api/share-links/copy?${params}`); if (!data.count) { toast("当前筛选下暂无可复制链接"); return; } await copyText(data.content.join("\n"), `已复制 ${data.count} 个链接`); } catch (error) { toast(error.message, true); } finally { button.disabled = false; } }
...
$("#password-form").addEventListener("submit", async (event) => { event.preventDefault(); const payload = { current_password: $("#current-password").value.trim(), new_password: $("#new-password").value.trim(), confirm_password: $("#confirm-password").value.trim() }; try { await api("/api/settings/password", { method: "POST", body: JSON.stringify(payload) }); event.currentTarget.reset(); showLogin(); toast("密码已更新，请使用新密码重新登录"); } catch (error) { toast(error.message, true); } });
$("#copy-webhook").addEventListener("click", () => copyText($("#webhook-url").textContent, "Webhook 地址已复制"));
$("#copy-share-links-api").addEventListener("click", () => copyText($("#share-links-copy-url").textContent, "密钥复制接口已复制"));
