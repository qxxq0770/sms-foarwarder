...
- 密钥历史展示密钥、有效期和“待使用/已使用”状态，页面默认筛选“待使用”，“已使用”以红色标签强调；支持按状态筛选并一键复制当前筛选下的全部链接，多个链接使用换行符分隔；只读密钥接口支持按状态和 12/24 小时有效期筛选，也可使用 Webhook Bearer Token
...
- 管理端按当前可见模块每 5 秒自动刷新数据，切回浏览器标签页时立即刷新；号池编辑中跳过自动重绘以保护未保存输入
