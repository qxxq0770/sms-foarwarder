...
- 一次生成 1–20 个密钥，使用或撤销前持续有效，历史列表可直接查看完整密钥
...
- 首次打开并成功分配号码后开始 12/24 小时倒计时，每个链接最多接收 2 次验证码；公开领取页只突出显示最新验证码，收到第 2 次后结束接收，但号码占用保留到链接过期
...
3. 在“密钥”中一次生成 1–20 个密钥，并立即复制本次返回的链接。
...
公开领取页收到验证码后显示醒目的 `NEW` 状态并显示 60 秒倒计时；超过 60 秒后状态切换为“已过期”，验证码区域保持白色背景并继续显示最近一条验证码。第 2 次验证码到达后该链接结束接收，后续新码不会再覆盖该链接。

分享 Token 位于 URL Fragment（`/c#t=...`），交换为短期 HttpOnly 会话后仍保留在地址栏，便于识别和再次打开完整链接。公开页后续领取和轮询会在存在 Fragment Token 时继续带 Bearer 头，避免同一浏览器短时间打开多个卡密时被全局 Cookie 会话串页；无 Fragment Token 时仍可使用已交换的 HttpOnly 会话。数据库保存用于验证的 Token 摘要，以及使用 Fernet 加密的原始 Token；完整值只通过已认证、禁止缓存的管理接口返回，不写入日志。升级前生成的旧密钥未保存原始值，无法恢复。
...
数据库位于 Docker 卷 `sms_data`，启动时按版本执行原子迁移。v4 会移除服务表和服务外键；v5 会移除密钥的领取截止时间、号码外键和任务外键；v7 为新密钥增加加密原始值；v10 支持一条短信同步到多个同号码有效任务；v11 会恢复尚未过期但因旧 3 条上限完成的任务；v12 将单链接验证码次数限制为 2，并结束已达到次数的活动任务；v13 调整号码占用统计，使已完成但未过期的链接继续占用号码。迁移保留已有短信、号码、密钥、任务和验证码，已创建的任务保持原号码。升级前建议同时安全备份 SQLite 卷与 `ENCRYPTION_KEY`。
