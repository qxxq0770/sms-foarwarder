...
- 密钥历史展示密钥、有效期和“待使用/已使用”状态，“已使用”以红色标签强调；支持按状态筛选并一键复制当前筛选下的全部链接，多个链接使用制表符分隔
- 首次打开并成功分配号码后开始 12/24 小时倒计时，每个窗口最多记录 3 条验证码；公开领取页只突出显示最新验证码
...
5. iOS 快捷指令在 Webhook 中提供 `message` 和 `recipient`；有效窗口内匹配接收号码与全局验证码规则的内容才会显示给访问者，新的验证码会覆盖公开页旧验证码。

公开领取页收到验证码后显示醒目的 `NEW` 状态并显示 60 秒倒计时；超过 60 秒后状态切换为“已过期”，验证码区域保持白色背景并继续显示最近一条验证码，直到收到下一条新验证码覆盖。

分享 Token 位于 URL Fragment（`/c#t=...`），交换为短期 HttpOnly 会话后仍保留在地址栏，便于识别和再次打开完整链接。数据库保存用于验证的 Token 摘要，以及使用 Fernet 加密的原始 Token；完整值只通过已认证、禁止缓存的管理接口返回，不写入日志。升级前生成的旧密钥未保存原始值，无法恢复。
...
数据库位于 Docker 卷 `sms_data`，启动时按版本执行原子迁移。v4 会移除服务表和服务外键；v5 会移除密钥的领取截止时间、号码外键和任务外键；v7 为新密钥增加加密原始值。迁移保留已有短信、号码、密钥、任务和验证码，已创建的任务保持原号码。升级前建议同时安全备份 SQLite 卷与 `ENCRYPTION_KEY`。
